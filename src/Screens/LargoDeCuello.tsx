...
import { Text, View, TextInput, Button, ScrollView, Alert, Dimensions, StyleSheet, TouchableOpacity, ImageBackground, Platform, PermissionsAndroid, Animated, Easing, Pressable } from "react-native";
...
export default function LargoDeCuello({route}) {
    ...
    const neckLength = async () => {
        try {
            ...
            const resp = await newApi.post('users/neckLength', {"neckLength": dataLogin} )
            console.log("camisasApi", resp.data);
            if (resp) {
                !route?.params?.actualizar  && navigation.navigate("TipoDeCuerpo", {category:categoryName})
            }
        } catch (error) {
            console.log(error);
        }
            ...
            >
                ...
                <View
                    style={{
                        ...
                        borderRadius: 10, marginBottom: 5, flexDirection: "row", alignItems: "center", marginTop: 30
                    }}
                >
                    <TextInput
                        style={{
